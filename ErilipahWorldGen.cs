...
namespace Erilipah
{
    public partial class ErilipahWorld : ModWorld
    {
        ...
        {
            while (!WorldGen.SolidOrSlopedTile(Main.tile[x, y]) || !(
        ...
        private void LostCityGen(GenerationProgress progress)
        {
            ...
            for (int passTheBeer = Chasm.Left + 60; passTheBeer < Chasm.Right - 60;)
            {
                int y = GetY(passTheBeer, Chasm.Top, mod.TileType<TaintedRubble>()) + 4;
        ...
        private void MakeHouse(int leftX, int rightX, int baseY, int floorHeight, int floors, ref int passTheBeer)
        {
            ...
            for (int floor = baseY; floor >= roofY; floor -= floorHeight)
            {
                ...
                {
                    WorldGen.PlaceBanner(leftX + 2, floor + 1, (ushort)mod.TileType<CityBanner>());
                    WorldGen.PlaceBanner(rightX - 3, floor + 1, (ushort)mod.TileType<CityBanner>());
                }

                // Modded paintings
                bool modded = WorldGen.genRand.Chance(0.15f);
                bool painting = WorldGen.genRand.Chance(paintingChance);
                if (modded && floor != baseY && painting)
                {
                    const int maxStyles = 1;
                    WorldGen.Place6x4Wall(
                        WorldGen.genRand.Next(leftX + 3, rightX - 3),
                        WorldGen.genRand.Next(floor + 3, floor + 8),
                        (ushort)mod.TileType<CityPainting>(), WorldGen.genRand.Next(maxStyles)
                        );
                }
                ...
                if (WorldGen.genRand.Chance(chestChance))
                {
                    ...
                    int index = WorldGen.PlaceChest(x, floor - 1);
                    if (index <= 0)
                        continue;

                    Chest chest = Main.chest[index];
                    chest.item[0] = mod.GetItem("ErilipahsUltimatum").item;
                }
            }

            // 70% chance to extend to another house out to the right
            if (rightX < Chasm.Right && roofY > Chasm.Y && WorldGen.genRand.Chance(0.70f))
            {
                ...
                minFloors += 2;
                int newFloors = Math.Max(minFloors, floors + Main.rand.Next(-2, 3));
                ...
                int extensions = WorldGen.genRand.NextBool(2) ? 2 : 1;
                if (floors > 4 && WorldGen.genRand.NextBool(2)) extensions++;
                if (floors > 7 && WorldGen.genRand.NextBool(2)) extensions++;
                for (int Y = 0; Y < extensions; Y++)
                {
                    if (Y == 1)
                    {
                        int lastExtension = floorToExtendY;
                        while (lastExtension == floorToExtendY) // Ensure that it isn't the same goddamn floor
                            floorToExtendY = baseY - floorHeight * WorldGen.genRand.Next(0, floors);
                    }
                    ...
                    // Banners
                    if (extensionRoof != baseY && WorldGen.genRand.Chance(bannerChance))
                    {
                        WorldGen.PlaceBanner(relLeft + 2, extensionRoof + 1, (ushort)mod.TileType<CityBanner>());
                        WorldGen.PlaceBanner(relRight - 3, extensionRoof + 1, (ushort)mod.TileType<CityBanner>());
                    }
                    ...
                    if (WorldGen.genRand.Chance(chestChance))
                    {
                        int x = WorldGen.genRand.Next(relLeft + 4, relRight - 4);
                        int index = WorldGen.PlaceChest(x, floorToExtendY - 1);
                        if (index <= 0)
                            continue;
        ...
        internal void SanguineOre()
        {
            for (int j = (int)WorldGen.rockLayer; j < WorldGen.lavaLine; j++)
            {
                int i = AnywhereX();
                if (WorldGen.SolidOrSlopedTile(Main.tile[i, j]))
                    WorldGen.OreRunner(i: i, j: j, strength: 8, steps: 5, type: T("SanguineTileTile"));
            }
        }
